...
app = FastAPI(
    title= "Advertising Sales Predictor API",
    description= (
        "API simple de Machine Learning (Regresion Lineal) entrenada con el dataset "
        "Advertising.csv (TC, Radio, Newspaper -> Sales). Soporta reentrenamiento y prediccion"
    ),
    version= "0.0.1"
)
...
def _ensure_model_exists():
    "Si no existe el modelo guardado, entrena uno con el dataset por defecto al arrancar"
    try:
        ml.load_model()
...
def root():
    return {
        "mensaje": "Advertising Sales Predictor API",
        "docs": "/docs",
...
def metrics():
    ...
    if metadata is None:
        return JSONResponse(
            status_code=404,
            content={"detalles":"No hay modelo entrenado todavia. LLama a /train"}
        )
